...
void test_datrie_states()
{
  ...
  datrie = create_datrie(wordimage, NULL, get_userdata_count(trie), 1);
...
void test_datrie()
{
  ...
  datrie = makeDatrieTree(65539, NULL);
  userdata.freq = 1;
  addCnWord(datrie, "中国人", &userdata);
  userdata.freq = 2;
  addCnWord(datrie, "你好啊", &userdata);
  userdata.freq = 3;
  addCnWord(datrie, "口碑网", &userdata);
  userdata.freq = 4;
  addCnWord(datrie, "阿里巴巴", &userdata);
  userdata.freq = 5;
  addCnWord(datrie, "阿里人", &userdata);
  userdata.freq = 6;
  addCnWord(datrie, "垂直搜索", &userdata);
  userdata.freq = 7;
  addCnWord(datrie, "垂直房产", &userdata);
  userdata.freq = 8;
  addCnWord(datrie, "百度", &userdata);
  userdata.freq = 9;
  addCnWord(datrie, "baby", &userdata);
  
  buildDatrie(datrie, 2);

  r = findCnWord(datrie, "中国人", &userdata);
  CHECK("check find 中国人", r == 2 && userdata.freq == 1);
  r = findCnWord(datrie, "你好啊", &userdata);
  CHECK("check find 你好啊", r == 2 && userdata.freq == 2);
  r = findCnWord(datrie, "口碑网", &userdata);
  CHECK("check find 口碑网", r == 2 && userdata.freq == 3);
  r = findCnWord(datrie, "阿里巴巴", &userdata);
  CHECK("check find 阿里巴巴", r == 2 && userdata.freq == 4);
  r = findCnWord(datrie, "阿里人", &userdata);
  CHECK("check find 阿里人", r == 2 && userdata.freq== 5);
  r = findCnWord(datrie, "垂直搜索", &userdata);
  CHECK("check find 垂直搜索", r == 2 && userdata.freq == 6);
  r = findCnWord(datrie, "垂直房产", &userdata);
  CHECK("check find 垂直房产", r == 2 && userdata.freq == 7);
  r = findCnWord(datrie, "百度", &userdata);
  CHECK("check find 百度", r == 2 && userdata.freq == 8);
  r = findCnWord(datrie, "baby", &userdata);
  CHECK("check find baby", r == 2 && userdata.freq == 9);
  r = findCnWord(datrie, "百度贴吧", &userdata);
  CHECK("check find 百度贴吧", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "百", &userdata);
  CHECK("check find 百", r == 1 && userdata.freq == 0);
  r = findCnWord(datrie, "电费单", &userdata);
  CHECK("check find 电费单", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "垂直", &userdata);
  CHECK("check find 垂直", r == 1 && userdata.freq == 0);
  r = findCnWord(datrie, "阿里人民", &userdata);
  CHECK("check find 阿里人民", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "口碑网络", &userdata);
  CHECK("check find 口碑网络", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "房产", &userdata);
  CHECK("check find 房产", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "垂询房产", &userdata);
  CHECK("check find 垂询房产", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "bab", &userdata);
  CHECK("check find bab", r == 1 && userdata.freq == 0);
  r = findCnWord(datrie, "babye", &userdata);
  CHECK("check find babye", r == 0 && userdata.freq == 0);
  r = findCnWord(datrie, "cabye", &userdata);
