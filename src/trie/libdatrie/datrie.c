...
struct datrie_s* create_datrie(struct wordimage_s* wordimage, struct datrieevent_s* event, int udsize, int scantype)
{
...
static void realloc_array(struct datrie_s* datrie)
{
  ...
  /* base array */
  allocsize = sizeof(int) * (datrie->size + datrie->encodesize);
