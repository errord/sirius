...
struct Z {
  D k, q, l;
  Z(D a=0,D b=-1,D c=0) : k(a), q(c), l((b<0) ? 1 : log(b))
  {};
};
...
int main(int argc,char**av)
{
  ...
  ifstream *muf=0;
  int mfc=-2,kfl=0;
  size_t st=0,C=0,it=0,maxIt=1000,ts=0,noF=0,Es,I,N=100000000;
  D ssi=0.0;
  ...
  for(int i=1;i<argc;++i) {
    ...
    if(si=="-v"||si=="-V")
      ...
      ssi=1.0/pow(atof(av[++i]),2);
    else if(si=="-kw"||si=="-kr")
      kfl=1+(si=="-kr");
    else if(si=="-unseenFeaturesZero")
      unseenFeaturesZero=1;
    else if(av[i][0]=='-') {
      ...
      case 1:
        if(s=="TEST") {
          ...
          for(size_t c=0;c<C;++c)
            e.Ny[c]=(c==e.y)?(1.0-dSmoothN):(dSmoothN/(C-1));
        }
        break;
      case 2:
        if(s=="#")
  ...
  if( mfc!= -2) {
    ...
    for(size_t n=0;n<Es;++n) {
      ...
      for(size_t i=0;i<C;++i) {
        cout << "@ " << en.Ny[i] << " ";
        for(size_t j=0;j<en.f[i].size();++j)
          if(s2f[en.f[i][j].first].second>mfc)
    ...
    do {
      ...
      for(size_t i=0;i<I;++i) {
        ...
        if(x.k) {
          ...
          if(ssi!=0.0)
            for (int iter=0;iter<20&&ddl>1e-10;++iter,dl-=ddl) {
      ...
      if(v>1)
        cerr<<it<< ". "<<" KLQ:"<<z<< " " << p<<"\n";
      if(TRN&&!qt)
        cerr<<it<<". "<<"pp: "<<exp(l/TRN)<<" er: "<<w/TRN<<" erx: " << wx/TRN << 
          " ppx: " << exp(lx/TRN);if(!TST)cerr<<endl;
      if(TST)
